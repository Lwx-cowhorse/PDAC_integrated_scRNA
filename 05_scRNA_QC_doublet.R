...
# ★ GSE155698 样本来源过滤：该数据集包含 PDAC_TISSUE、PDAC_PBMC、
# Healthy_PBMC、AdjNorm_TISSUE 四类样本。仅保留 PDAC 肿瘤组织。
# 参考文献: Steele et al. 2020 免疫图谱研究，实验设计包含 PBMC
filter_gse155698_tissue <- function(obj) {
  # 查找包含 "PBMC" 或 "TISSUE" 关键词的 metadata 列
  sample_col <- NULL
  for (col in colnames(obj@meta.data)) {
    vals <- as.character(obj@meta.data[[col]])
    if (any(grepl("PBMC|TISSUE|Healthy|AdjNorm", vals, ignore.case = TRUE))) {
      sample_col <- col
      break
    }
  }
  if (is.null(sample_col)) {
    cat("  [WARN] GSE155698: no sample-source column found, keeping all cells\n")
    return(obj)
  }
  nc_before <- ncol(obj)
  # 仅保留 PDAC_TISSUE（排除 PBMC、Healthy、AdjNorm）
  keep <- grepl("PDAC_TISSUE", as.character(obj@meta.data[[sample_col]]), ignore.case = TRUE)
  obj <- subset(obj, cells = colnames(obj)[keep])
  cat(sprintf("  GSE155698 %s filter: %d -> %d (removed %d PBMC/normal/adjacent cells)\n",
              sample_col, nc_before, ncol(obj), nc_before - ncol(obj)))
  return(obj)
}
obj2 <- filter_gse155698_tissue(obj2)
